...
BLOCK_MARKER_START = "; >>> ARK_SURVIVAL_DESCENDED_START <<<"
BLOCK_MARKER_END   = "; >>> ARK_SURVIVAL_DESCENDED_END <<<"
LEGACY_BLOCK_MARKER_START = "; >>> ASE_PURIST_INJECTOR_START <<<"
LEGACY_BLOCK_MARKER_END   = "; >>> ASE_PURIST_INJECTOR_END <<<"
...
def generate_replacement_block(blocked_classes: set):
    ...
    lines = [
        BLOCK_MARKER_START,
        "; Generated by Ark Survival Descended on " + datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
...
def strip_existing_blocks(content: str) -> str:
    """Remove existing blocks from content (supports current and legacy markers)."""
    for start_m, end_m in [
        (BLOCK_MARKER_START, BLOCK_MARKER_END),
        (LEGACY_BLOCK_MARKER_START, LEGACY_BLOCK_MARKER_END)
    ]:
        if start_m in content and end_m in content:
            start_idx = content.find(start_m)
            end_idx = content.find(end_m) + len(end_m)
...
def inject_into_game_ini(file_path: Path, replacement_block: str):
    ...
    # Strip existing blocks if present
...
def remove_from_game_ini(file_path: Path):
    ...
    has_marker = any(
        start_m in content and end_m in content
        for start_m, end_m in [
            (BLOCK_MARKER_START, BLOCK_MARKER_END),
            (LEGACY_BLOCK_MARKER_START, LEGACY_BLOCK_MARKER_END)
        ]
    ...
    if has_marker:
        new_content = strip_existing_blocks(content)
        with open(file_path, "w", encoding="utf-8") as f:
            f.write(new_content)
        print(f"[✓] Successfully removed Ark Survival Descended blocks from: {file_path}")
    else:
        print("[!] No Ark Survival Descended block was found in this file.")
...
def print_banner(target_file):
    print("=" * 66)
    print("          🦖 ARK: Survival Ascended - Ark Survival Descended 🦕")
    print("=" * 66)
...
def interactive_main_menu():
    ...
    while True:
        ...
        if choice == '1':
            ...
            print("\nExiting Ark Survival Descended Manager. Happy surviving! 🦖")
            sys.exit(0)
...
def main():
    parser = argparse.ArgumentParser(
        description="ARK: Survival Ascended - Ark Survival Descended Game.ini Manager"
    )
    parser.add_argument(
        "file",
        nargs="?",
    ...
    parser.add_argument(
        "--remove",
        action="store_true",
        help="Headless mode: Remove previously injected Ark Survival Descended lines from Game.ini"
    )
    ...
    if args.all or args.allow or args.remove:
        ...
        if not target or not target.is_file():
            ...
            print(f"[ERROR] Game.ini not found. Specify path: python {script_name} <path>")
            sys.exit(1)
