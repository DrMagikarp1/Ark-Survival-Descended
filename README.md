...
The original scope of this project was conceived as an in-engine DevKit mod. However, during development, I found that using a standalone **Python script** to manage native Unreal Engine `Game.ini` entries is vastly superior (and also I don't have enough storage to keep the devkit on my pc)
1. **No Mod Installation Required:** Works immediately on vanilla clients and dedicated servers without requiring players to subscribe and use any CurseForge mod.
2. **Zero Performance Overhead:** Uses the game's native `NPCReplacements` inside the 'game.ini'. Spawns are blocked directly at the engine level, so there should be no performance hit. 
3. **Safe for Save Games:** This script only effects new spawns, so if you have tamed creatures prior to this mod they should not be affected by this.
...
3. **Total User Control:**
   ...
   * Easily toggle *Garuga's ARK Additions*, *Bob's Tall Tales*, *Fantastic Tames*, and community vote creatures.
...
* **Bob's Tall Tales:** Oasisaur, Cosmo (`JumpingSpider`), Sir-5rM-8 (`HelperBot`), Armadoggo (`Doggo`), Zeppelin
* **Fantastic Tames (Paid DLC):** Pyromane (`FireLion`), Dreadmare (`DarkPegasus`), Burrowbuck (`Jackalope`), Elderclaw (`SpiritBear`)
* **ASA Community Votes & Story Newcomers:** Fasolasuchus, Gigantoraptor, Shastasaurus, Yi Ling, Bison, Dreadnoughtus, Boaratos, Grand Tortugar, Maelizard
* **Garuga Official Additions:** Ceratosaurus, Deinosuchus, Archelon, Brachiosaurus, Xiphactinus, Helicoprion, Acrocanthosaurus, Concavenator, Deinotherium *(Configurable!)*
* **Expansion DLCs:** Lost Colony (Aureliax, Cryolophosaurus, Ossidon, Solwyn, Veilwyn, Drakelings, Thralls), Dragontopia (Draco, Eclipsar, Lumina), Tides of Fortune (Axolotl / Tidepups, Parrot, Palaeoctopus)(Yes this will block these spawns on Lost Colony map, so don't use it there!)
